...
public class CommitRollout {
  ...
  public static void commitRollout(String deploymentName)
    ...
    RetryingFuture<OperationSnapshot> poll =
        client.commitRolloutAsync(deploymentName).getPollingFuture();

    OperationSnapshot response = poll.get(1, TimeUnit.MINUTES);
    if (response.isDone()) {
      System.out.println("Rollout committed: " + response.getResponse());
    } else {
      throw new RuntimeException("Commit Rollout request unsuccessful.");
    }
  }
}
